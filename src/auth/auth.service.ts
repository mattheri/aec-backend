...
export class AuthService {
  ...
  private async createUser(createUserDto: CreateUserDto) {
    try {
      ...
      if (error.message.includes(constants.messages.DUPLICATE_KEY_ERROR))
  ...
  private async validateUserPassword(user: User, password: string) {
    const passwordHash = await this.encryptionService.compare(
      password,
      user.hash,
    );
    if (!passwordHash)
      throw new HttpException(
        UserErrorCodes.USER_PASSWORD_INCORRECT,
  ...
  async login(findUserDto: FindUserDto) {
    const foundUser = await this.usersService.findOneWithHash({
    ...
    const updatedWithRefreshTokenUser = await this.usersService.update(
  ...
  async refresh(token: string, username: string) {
    if (!token)
      throw new HttpException(
        UserErrorCodes.TOKEN_NOT_PROVIDED,
        HttpStatus.UNAUTHORIZED,
      );
    if (!username)
      throw new HttpException(
        UserErrorCodes.USERNAME_NOT_PROVIDED,
        HttpStatus.UNAUTHORIZED,
      );
  ...
  async getTokens(username: string) {
    const accessToken = await this.jwtService.signAsync(
      { username },
      {
        expiresIn: this.configService.get(constants.config.JWT_EXPIRATION_KEY),
        secret: this.configService.get(constants.config.JWT_SECRET_KEY),
      },
    );
    const refreshToken = await this.jwtService.signAsync(
      { username },
      {
        expiresIn: this.configService.get(
          constants.config.REFRESH_JWT_EXPIRATION_KEY,
        ),
        secret: this.configService.get(constants.config.REFRESH_JWT_SECRET_KEY),
      },
    );
