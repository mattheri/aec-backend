...
export class UsersService {
  ...
  async create(createUserDto: CreateUserDto): Promise<User> {
    ...
    const user = (
      await this.userModel.create({ ...createUserDto, hash })
    ).toObject();
    await this.userConfigService.create({ user: user._id });
  ...
  async findOne({ username, _id }: FindOneUserArguments) {
    if (!username && !_id)
      throw new HttpException(
        UserErrorCodes.NO_ID_OR_USERNAME,
  ...
  async update(query: FindOneUserArguments, updateUserDto: UpdateUserDto) {
    ...
    const user = (
      await this.userModel.findOneAndUpdate(query, updateUserDto, { new: true })
    ).toObject();
    return new UserEntity(user);
  }
